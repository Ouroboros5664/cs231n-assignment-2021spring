...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, norm, gamma, mean, var, std)
        pass
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, norm, gamma, mean, var, std = cache
    N, D = dout.shape
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(norm * dout, axis=0)

    #compute dx1, dx2, dx3, dx
    dhatx = dout * gamma
    dx1 = dhatx / std

    dsigma = -0.5 * dhatx * (x - mean) * (std ** -3)
    dsigma = np.sum(dsigma, axis=0)
    dxx = 2 * (x - mean) / N
    dx2 = dsigma * dxx

    dmu1 = dhatx
    dmu2 = dsigma
    tmp = -2*np.sum(x-mean,axis=0)/N * dmu2
    dmu = np.sum(-1 * dmu1 / std, axis=0) + np.sum(tmp, axis=0)    
    dx3 = dmu / N * np.ones((N, D))
...
def batchnorm_backward_alt(dout, cache):
    ...
    dhatx = dout * gamma
    dsigma = np.sum(dhatx * (x - mean) * -0.5 * (var ** -1.5), axis=0)
    dmu = np.sum(dhatx * -1 / std, axis=0) + dsigma * -2 * np.sum(x - mean, axis=0) / N
    dx = dhatx / std + dsigma * 2 * (x - mean) / N + dmu / N
...
def layernorm_backward(dout, cache):
    ...
    dhatx = dout * gamma
    dsigma = np.sum(dhatx * (x - mean) * -0.5 * (var ** -1.5), axis=1)
    dmu = np.sum(dhatx * -1 / std, axis=1) + dsigma * -2 * np.sum(x - mean, axis=1) / N
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W = x.shape
    x_trans = np.transpose(x, (0,2,3,1)).reshape(-1,C)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W = dout.shape
    dout_trans = np.transpose(dout, (0,2,3,1)).reshape(-1,C)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dgamma = np.sum(x_norm * dout, axis=(0,2,3)).reshape(1,C,1,1)
    x_trans = x.reshape(N, G, C//G, H, W)
    
    dhatx = dout * gamma
    dhatx = dhatx.reshape(N, G, C // G, H, W)
    M = C // G * H * W
    dsigma = np.sum(dhatx * (x_trans - mean) * -0.5 * (var ** -1.5), axis=(2,3,4))
    dmu = np.sum(dhatx * -1 / std, axis=(2,3,4)) + dsigma * -2 * np.sum(x_trans - mean, axis=(2,3,4)) / M
    dsigma = dsigma.reshape(N, G, 1, 1, 1)
    dmu = dmu.reshape(N, G, 1, 1, 1)
    dx = dhatx / std + dsigma * 2 * (x_trans - mean) / M + dmu / M
